...
<body>
    ...
    <div class="container mt-5">
        <div class="row g-4">
            ...
            <div class="col-lg-8">
                ...
                @if($soalKuis->isEmpty())
                    <div class="alert alert-info text-center">Belum ada soal untuk Evaluasi.</div>
                @else
                ...
                @if(!$soalKuis->isEmpty())
                    <div class="card panel-nomor">
                        ...
                        <div class="card-body text-center">
                            ...
                            @foreach($soalKuis as $index => $soal)
                                <button type="button" id="btn-soal-{{ $index + 1 }}" class="nomor-soal soal-belum"
                                    onclick="lompatSoal({{ $index }})">{{ $index + 1 }}</button>
                            @endforeach
                            ...
                            <hr>
                            <button class="btn btn-success w-100 py-2 fw-bold" id="finishSidebarBtn" onclick="validasiSelesai()">✅ Selesai Evaluasi</button>
    ...
    <form action="{{ route('simpan-nilai') }}" method="POST" id="form-submit" style="display:none;">
        ...
        <input type="text" name="type" value="Evaluasi">
    ...
    <script>
        ...
        function tampilkanSoal(i) {
            ...
            if (document.getElementById(`btn-soal-${i + 1}`)) {
                document.getElementById(`btn-soal-${i + 1}`).classList.add('soal-aktif');
            }
            indexAktif = i;
            if (prevBtn) prevBtn.style.display = i === 0 ? 'none' : 'inline-block';
            if (nextBtn) nextBtn.style.display = i === soal.length - 1 ? 'none' : 'inline-block';
        }
        function lompatSoal(i) { tampilkanSoal(i); }
        if (prevBtn) prevBtn.onclick = () => tampilkanSoal(indexAktif - 1);
        if (nextBtn) nextBtn.onclick = () => tampilkanSoal(indexAktif + 1);
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 16px; border: 2px solid var(--ungu);">
          <div class="modal-header" style="background: var(--ungu); color: white; border-top-left-radius: 12px; border-top-right-radius: 12px;">
            <h5 class="modal-title fw-bold" id="modalKonfirmasiLabel">Konfirmasi Penyerahan Evaluasi</h5>
            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body text-center py-4">
            <h5 class="mb-3">Selesai Mengerjakan Evaluasi?</h5>
